...
\newcommand{\semanticBb}{${\lambda}{\backslash}_S$}
...
\[ s \oplus x \mapsto w = \begin{cases} 
      x \mapsto \{v\}& s =  \varnothing \wedge w = v\\
      x \mapsto w & s =  \varnothing \wedge w \neq v\\
      x \mapsto w' \cup \{v\}, s' & s = x \mapsto w', s' \wedge w = v\\
...
\label{definition:tcs}
\mbox{}
The operator $\filtrar$ is defined  as follows:
...
%$ \conf{\ascrip{t}} \tto \ascrip{t[s]}$&$\rulename{AscSub}$\\
%&\\
%$ \conf{\oletP{T_1}{t_1}{t_2}} \tto \oletP{T_1}{t_1[s]}{t_2[s]}$&$\rulename{LetSub} $\\
%&\\
%$\conf{\appD} \tto t_1 [s] \ t_2 [s]$&$\rulename{AppSub} $\\
%&\\
%$\conf{\suma{t_1}} \tto \suma{t_1 [s]}$&$\rulename{SumSub} $\\
...
%&\\
%$\inference {\filtrar(w, T_1) = \{v\}}{\oletP{T_1}{w}{t_2[s]} \tto \confext{t_2}}$&$\rulename{Let} $\\
%&\\
%${\oletP{T_1}{w}{t_2[s]} \tto \confextW{t_2}}$&$\rulename{Let} $\\
$\cdots$&\\
