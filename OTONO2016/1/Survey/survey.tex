...
Odersky et al.~\cite{Odersky1995} propose an extension of the Hindley/Milner system that supports overloading and polymorphic records. They proof that the type system is sound respect to a standard untyped compositional semantics. Additionally, the show  that every typable term in this system has  a principal type. They highlight that for type classes cannot be assigned a meaning independent of its types, so some important properties 
