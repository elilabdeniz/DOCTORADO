...
&\\
$ \inference{\buscar(x, [s], S, v)}{\ascripS{x[s]} \tto  \ascrip{v}} $&$\rulename{AscVar}$\\
&\\
...
\mbox{}
The function $\filtrar$ is defined  as follows:
\[ \filtrar(w, T) = \begin{cases} 
      w & w = v \\
      v' & ! \exists v' \in w |\lab(v') = T\\
   \end{cases}
\]
...
&&\\
&$\inference{\tyC \oletPT{T_1}{t_1[s]}{t_2[s] : \mtP{T}}}{\tyC \conf{\oletPT{T_1}{t_1}{t_2}} : \mtP{T}}$&$\rulename{CT Let}$\\
&&\\
